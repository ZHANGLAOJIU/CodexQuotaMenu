...
菜单栏里的数字表示“剩余百分比”。点击后可以查看两个额度窗口的重置倒计时、banked reset 可用次数、每次重置的准确到期时间、最近同步时间以及当前数据来源。
...
- 点击后显示原生彩色进度条、剩余比例和重置倒计时。
- 同时显示 5 小时和一周两个额度窗口。
- 显示 banked reset 可用次数，并逐条显示每次 `Full reset` 精确到秒的本地到期时间。
- Banked resets 按最早到期时间排序，只展示状态为 `available` 的项目。
- 官方接口异常时自动读取本地 Codex 响应日志作为备用。
- 明确标记“官方接口”或“本地日志”，不会把旧数据伪装成实时数据。
- 根据窗口时长识别 5 小时与一周额度，OpenAI 临时关闭某个窗口时不会颠倒显示。
...
应用会从 `~/.codex/auth.json` 读取 Codex 已有的访问令牌和账号 ID，并仅向下面两个 Codex 官方地址查询用量和 banked resets：
...
程序不会把 `primary_window` 永久当作 5 小时额度。OpenAI 在测试期间可能把唯一的一周额度放进 `primary_window`；程序会根据 300 分钟或 10080 分钟的窗口时长分类，缺失的窗口显示 `--%`。
